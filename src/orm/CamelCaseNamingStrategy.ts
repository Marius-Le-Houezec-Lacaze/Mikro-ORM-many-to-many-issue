...
export class CamelCaseNamingStrategy extends AbstractNamingStrategy {
  ...
  joinKeyColumnName(entityName: string, referencedColumnName?: string): string {
    return camelCase(this.classToTableName(entityName) + '_' + (referencedColumnName || this.referenceColumnName()));
  }

  joinTableName(sourceEntity: string, targetEntity: string, propertyName?: string): string {
    return camelCase(this.classToTableName(sourceEntity) + 'Has' + this.capitalize(targetEntity));
  }
